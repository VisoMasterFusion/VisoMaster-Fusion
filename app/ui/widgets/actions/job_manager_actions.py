...
def convert_parameters_to_job_type(
    ...
    parameters: dict | ParametersTypes,
    convert_type: type
...
def save_job(
    ...
    try:
        ...
        QMessageBox.critical(main_window, "Save Error", f"Failed to save job '{job_name}':\n{e}")
    ...
    for job_name in selected_jobs:
        ...
        if os.path.exists(job_file):
            try:
                ...
                QMessageBox.warning(main_window, "Delete Error", f"Could not move job '{job_name}' to trash:\n{e}")
        else:
...
def _clear_main_window_state(main_window: "MainWindow"):
    ...
    video_control_actions.remove_all_markers(main_window) # Clear markers from slider and data
...
def _load_job_target_media(main_window: "MainWindow", data: dict):
    ...
    for m in target_medias_data:
        if "media_path" in m and os.path.exists(m["media_path"]):
            ...
            print(f"[WARN] Target media path not found, skipping: {m.get('media_path')}")
...
def _load_job_input_faces(main_window: "MainWindow", data: dict):
    ...
    for face_id, f in input_faces_data.items():
        if "media_path" in f and os.path.exists(f["media_path"]):
            ...
            print(f"[WARN] Input face media path not found, skipping: {f.get('media_path')}")
...
def _load_job_target_faces_and_params(main_window: "MainWindow", data: dict):
    ...
    for face_id_str, target_face_data in loaded_target_faces_data.items():
        ...
        if face_id in main_window.target_faces:
            ...
            print(f"[WARN] Target face object with id {face_id} not found after creation.")

def _load_job_controls_and_state(main_window: "MainWindow", data: dict, is_batch_load: bool = False):
        ...
        for media in data.get("target_medias_data", []):
            if media.get("media_id") == job_selected_media_id:
                ...
                if not media_path_to_check:
                    is_job_valid = False
                    skip_reason = f"Selected media ID {job_selected_media_id} has no media path."
        ...
        if is_job_valid:
            ...
            for embed_id in required_embed_ids:
                if embed_id not in all_embeddings_in_job:
                    is_job_valid = False
                    skip_reason = f"Required embedding ID {embed_id} not found in job data."
                    break
...
def load_job_workspace(main_window: "MainWindow", job_name: str):
    ...
    """
    from app.ui.widgets import widget_components # Local import for dialog
    ...
    if not Path(data_filename).is_file():
        ...
        QMessageBox.critical(main_window, "Load Error", f"Job file not found:\n{data_filename}")
        return
    ...
    try:
        ...
        QMessageBox.critical(main_window, "Load Error", f"Failed to load job '{job_name}':\n{e}")
        return
    ...
    if not is_valid:
        ...
        QMessageBox.critical(main_window, "Load Error", 
                             f"Failed to load job '{job_name}':\n\n{error_msg}")
        return # Abort loading

    # --- Show Progress Dialog ---
    steps = [
        "Clearing State", "Loading Target Media", "Loading Input Faces",
        "Loading Embeddings", "Loading Target Faces", "Loading Settings",
        "Loading Markers", "Finalizing"
    ]
    ...
    try:
        ...
        if main_window.target_faces:
            ...
            if first_face_button:
                 ...
                 print(f"[DEBUG] Set cur_selected_target_face_button to face_id: {first_face_id}")

                 # *** Visually check assigned embeddings for this face ***
                 print(f"[DEBUG] Checking assigned embeddings for face_id: {first_face_id}")
                 assigned_embedding_ids = first_face_button.assigned_merged_embeddings.keys()
                 for embed_id in assigned_embedding_ids:
                     ...
                     if embed_button:
                         ...
                         print(f"  - Checked embedding: {embed_button.embedding_name} (ID: {embed_id})")

                 # *** Visually check assigned input faces for this face ***
                 print(f"[DEBUG] Checking assigned input faces for face_id: {first_face_id}")
                 ...
                 for input_face_id in assigned_input_face_ids:
                     ...
                     if input_face_button:
                         ...
                         print(f"  - Checked input face: {input_face_button.media_path} (ID: {input_face_id})")

            print(f"[DEBUG] Setting parameter widgets for loaded face_id: {first_face_id}")
            ...
            main_window.current_widget_parameters = main_window.parameters[first_face_id].copy()
            ...
            main_window.current_widget_parameters = main_window.default_parameters.copy()
            print("[DEBUG] No target faces loaded in job, parameter widgets retain default values.")
        ...
        traceback.print_exc()
        QMessageBox.critical(main_window, "Load Error", f"An error occurred while loading job '{job_name}':\n{e}")
    finally:
...
def _restore_workspace_from_snapshot(main_window: "MainWindow", data: dict):
    ...
    if not data:
        ...
        QMessageBox.critical(main_window, "Restore Error", "Failed to restore workspace: Snapshot data was empty.")
        return

    # --- Show Progress Dialog ---
    steps = [
        "Clearing State", "Restoring Target Media", "Restoring Input Faces",
        "Restoring Embeddings", "Restoring Target Faces", "Restoring Settings",
        "Restoring Markers", "Finalizing"
    ]
    total_steps = len(steps)
    progress_dialog = widget_components.JobLoadingDialog(
        total_steps, parent=main_window
    ...
    try:
        ...
        if main_window.target_faces:
            ...
            if first_face_button:
                 ...
                 # *** Visually check assigned embeddings for this face ***
                 print(f"[DEBUG] Checking assigned embeddings for restored face_id: {first_face_id}")
                 assigned_embedding_ids = first_face_button.assigned_merged_embeddings.keys()
                 for embed_id in assigned_embedding_ids:
                     embed_button = main_window.merged_embeddings.get(embed_id)
                     if embed_button:
                 ...
                 # *** Visually check assigned input faces for this face ***
                 print(f"[DEBUG] Checking assigned input faces for restored face_id: {first_face_id}")
                 assigned_input_face_ids = first_face_button.assigned_input_faces.keys()
                 for input_face_id in assigned_input_face_ids:
                     input_face_button = main_window.input_faces.get(input_face_id)
                     if input_face_button:
            ...
            print(f"[DEBUG] Setting parameter widgets for restored face_id: {first_face_id}")
            # Now call the update function with the specific face_id
            common_widget_actions.set_widgets_values_using_face_id_parameters(
                main_window, face_id=first_face_id
            )
            ...
            main_window.cur_selected_target_face_button = None
            main_window.current_widget_parameters = main_window.default_parameters.copy()
            print("[DEBUG] No target faces restored in snapshot, parameter widgets retain default values.")
        ...
        print(f"[DEBUG] Workspace restored successfully from snapshot.")

    except Exception as e:
        print(f"[ERROR] Error during workspace snapshot restoration: {e}")
        traceback.print_exc()
        QMessageBox.critical(main_window, "Restore Error", f"An error occurred while restoring the workspace:\n{e}")
    finally:
        progress_dialog.close()
...
def update_job_manager_buttons(main_window: "MainWindow"):
    ...
    if hasattr(main_window, "loadJobButton"):
        main_window.loadJobButton.setEnabled(enable_on_single_selection) # Only allow loading one job
...
def prompt_job_name(main_window: "MainWindow"):
    ...
    if main_window.target_faces:
        for target_face in main_window.target_faces.values():
            ...
            if (len(target_face.assigned_input_faces) + len(target_face.assigned_merged_embeddings)) > 0:
...
def connect_job_manager_signals(main_window: "MainWindow"):
    ...
    if main_window.loadJobButton:
        main_window.loadJobButton.clicked.connect(lambda: load_job(main_window)) # Uses heavy load
...
def load_master_assets(main_window: "MainWindow", master_data: dict):
    ...
    # --- Show Progress Dialog ---
    steps = ["Clearing State", "Loading All Target Media", "Loading All Input Faces", "Loading All Embeddings"]
    total_steps = len(steps)
    ...
    try:
        ...
        traceback.print_exc()
        QMessageBox.critical(main_window, "Load Error", f"An error occurred while loading batch assets:\n{e}")
    finally:
        progress_dialog.close()
...
def load_job_settings(main_window: "MainWindow", job_data: dict):
    ...
    try:
        ...
        main_window.use_job_name_for_output = job_data.get("use_job_name_for_output", False)
        ...
        if selected_media_id and main_window.target_videos.get(selected_media_id):
            ...
            print(f"[WARN] (Main Thread) Could not select media_id {selected_media_id} for job.")
            ...
            if main_window.target_videos:
                ...
                print(f"[WARN] (Main Thread) Selecting first available media instead: {first_media.media_id}")
                first_media.click()
            else:
        ...
        print(f"[DEBUG] (Main Thread) Lightweight settings loaded for job: {main_window.current_job_name}")

    except Exception as e:
        print(f"[ERROR] (Main Thread) Error during job settings loading: {e}")
        traceback.print_exc()
        QMessageBox.critical(main_window, "Load Error", f"An error occurred while loading settings for job:\n{e}")
    finally:
        # Use the instance event from the job_processor
        if main_window.job_processor:
...
def clear_job_settings(main_window: "MainWindow"):
    ...
    try:
        card_actions.clear_target_faces(main_window)  # Clears target faces and parameters
        video_control_actions.remove_all_markers(main_window) # Clear markers from slider and data
        main_window.job_marker_pairs.clear() # Clear job segments
...
def handle_batch_completion(main_window: "MainWindow"):
    ...
    if snapshot:
        ...
        try:
            ...
            traceback.print_exc()
            QMessageBox.critical(main_window, "Restore Error", 
        ...
        print("[WARN] (Main Thread) No workspace snapshot found. UI will remain in its last state.")
    ...
    if skipped_jobs:
        ...
        QMessageBox.warning(
            main_window, "Skipped Jobs", skipped_message
        )

    # Report final status
    if batch_succeeded:
        QMessageBox.information(
            main_window, "Job Processing Complete", "All valid jobs have finished processing."
        )
    else:
        # Batch failed
        QMessageBox.warning(
            main_window, "Job Processing Failed", 
...
def start_job_processor(main_window: "MainWindow", jobs_to_process: list[str] | None):
    ...
    if main_window.job_processor and main_window.job_processor.isRunning():
        QMessageBox.warning(main_window, "Already Processing", "A job processor is already running.")
        return
    ...
    main_window.job_processor = JobProcessor(main_window, jobs_to_process=jobs_to_process)
    ...
    main_window.job_processor.job_completed_signal.connect(
        lambda job_name: refresh_job_list(main_window) # The signal sends job_name, refresh_job_list is called
    )
...
class JobProcessor(QThread):
    ...
    JOB_SETTINGS_LOAD_TIMEOUT = 180   # Max time to load a single job's settings (3 minutes)
    JOB_SETTINGS_CLEAR_TIMEOUT = 30   # Max time to clear a single job's settings
    
    def __init__(self, main_window: "MainWindow", jobs_to_process: list[str] | None = None):
        """
        ...
        self.batch_succeeded = False # Flag to track if the batch finished without errors
        ...
        self.main_window.video_processor.processing_heartbeat_signal.connect(
            self.handle_processing_heartbeat,
            Qt.DirectConnection
        )
    ...
    def _read_job_file(self, job_name: str) -> dict | None:
        ...
        if not Path(data_filename).is_file():
            print(f"[DEBUG] No valid file found for job: {job_name}.")
            self.job_failed_signal.emit(job_name, f"Job file not found: {data_filename}")
            return None
        try:
            with open(data_filename, "r") as data_file:
                data = json.load(data_file)
    ...
    def _analyze_job_batch(self) -> dict | None:
        ...
        """
        print("[DEBUG] (JobProcessor) Performing SMART analysis and VALIDATION of job batch...")
        master_data = {
            "target_medias_data": [],
            "input_faces_data": {},
            "embeddings_data": {}
        ...
        for job_name in self.jobs:
            ...
            if data is None:
                ...
                self.skipped_jobs.append(f"{job_name}: Job file could not be read or parsed.")
            ...
            if not is_job_valid:
                print(f"[WARN] (JobProcessor) Skipping job '{job_name}'. Reason: {skip_reason}")
            ...
            for face_id in required_face_ids:
                if face_id not in seen_face_ids:
                    ...
                    master_data['input_faces_data'][face_id] = all_input_faces_in_job[face_id]
            ...
            for embed_id in required_embed_ids:
                if embed_id not in seen_embed_ids:
                    ...
                    master_data['embeddings_data'][embed_id] = all_embeddings_in_job[embed_id]
        ...
        print(f"[DEBUG] (JobProcessor) Smart Analysis complete:")
        ...
        print(f"  - {len(master_data['target_medias_data'])} unique target media required.")
        print(f"  - {len(master_data['input_faces_data'])} unique input faces required.")
    ...
    def _trigger_and_wait_for_processing(self, job_name: str) -> bool:
        ...
        QMetaObject.invokeMethod(
            self.main_window.buttonMediaRecord,
            "toggle",
            Qt.QueuedConnection
        )
        ...
        print("[DEBUG] JobProcessor detected processing started. Waiting for completion with heartbeat...")
    ...
    def run(self):
        ...
        if master_data is None:
            print("[ERROR] Failed to analyze job batch (master_data is None). Aborting.")
        ...
        if not self.job_data_list:
            print("[DEBUG] No valid jobs found in batch after analysis. Skipping processing.")
        ...
        if not self.master_assets_loaded_event.wait(timeout=self.MASTER_ASSETS_LOAD_TIMEOUT): # Use instance event
        ...
        for job_data in self.job_data_list:
            ...
            if not self.job_settings_loaded_event.wait(timeout=self.JOB_SETTINGS_LOAD_TIMEOUT): # Use instance event
            ...
            if not self._trigger_and_wait_for_processing(job_name):
                ...
                print(f"[ERROR] Job '{job_name}' failed during processing. Aborting batch.")
            ...
            if not self.job_settings_cleared_event.wait(timeout=self.JOB_SETTINGS_CLEAR_TIMEOUT): # Use instance event
