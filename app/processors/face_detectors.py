...
class FaceDetectors:
    ...
    def __init__(self, models_processor: 'ModelsProcessor'):
        ...
        self.detector_map: Dict[str, Dict[str, Any]] = {
            'RetinaFace': {'model_name': 'RetinaFace', 'function': self.detect_retinaface},
    ...
    def _prepare_detection_image(self, img: torch.Tensor, input_size: tuple, normalization_mode: str) -> tuple[torch.Tensor, torch.Tensor, tuple]:
        """
        ...
        canvas_dtype = torch.float32 if normalization_mode in ['retinaface', 'scrfd'] else torch.uint8
        det_img_canvas = torch.zeros((input_size[1], input_size[0], 3), dtype=canvas_dtype, device=self.models_processor.device)
    ...
    def _filter_detections_gpu(self, scores_list, bboxes_list, kpss_list, img_height, img_width, det_scale, max_num):
        """
        ...
        scores_tensor = torch.from_numpy(np.vstack(scores_list)).to(self.models_processor.device).squeeze()
        bboxes_tensor = torch.from_numpy(np.vstack(bboxes_list)).to(self.models_processor.device)
        kpss_tensor = torch.from_numpy(np.vstack(kpss_list)).to(self.models_processor.device)
        ...
        if bboxes_tensor.numel() == 0: return None, None, None
        if bboxes_tensor.dim() == 1 and bboxes_tensor.numel() == 4: bboxes_tensor = bboxes_tensor.unsqueeze(0)
        if scores_tensor.dim() == 0: scores_tensor = scores_tensor.unsqueeze(0)
        if bboxes_tensor.size(0) != scores_tensor.size(0): return None, None, None
        ...
        det_boxes, det_kpss, det_scores = bboxes_tensor[keep_indices], kpss_tensor[keep_indices], scores_tensor[keep_indices]
        
        # Sort the remaining detections by their confidence score.
        sorted_indices = torch.argsort(det_scores, descending=True)
        det_boxes, det_kpss, det_scores = det_boxes[sorted_indices], det_kpss[sorted_indices], det_scores[sorted_indices]
        ...
        if max_num > 0 and det_boxes.shape[0] > max_num:
            ...
            det_boxes, det_kpss, det_scores = det_boxes[bindex], det_kpss[bindex], det_scores[bindex]
    ...
    def _refine_landmarks(self, img_landmark, det, kpss, score_values, use_landmark_detection, landmark_detect_mode, landmark_score, from_points, **kwargs):
        """
        ...
        if use_landmark_detection and len(kpss_5) > 0:
            ...
            for i in range(kpss_5.shape[0]):
                ...
                refined_kpss.append(landmark_kpss if len(landmark_kpss) > 0 else kpss_5[i])
                # If the new landmarks have a higher confidence, replace the old 5-point landmarks.
                if len(landmark_kpss_5) > 0 and (len(landmark_scores) == 0 or np.mean(landmark_scores) > np.mean(score_values[i])):
    ...
    def run_detect(self, img, detect_mode='RetinaFace', max_num=1, score=0.5, input_size=(512, 512), use_landmark_detection=False, landmark_detect_mode='203', landmark_score=0.5, from_points=False, rotation_angles=None):
        ...
        args = {
            'img': img, 'max_num': max_num, 'score': score, 'use_landmark_detection': use_landmark_detection,
            'landmark_detect_mode': landmark_detect_mode, 'landmark_score': landmark_score, 'from_points': from_points,
    ...
    def detect_retinaface(self, **kwargs):
        ...
        img, input_size, score, rotation_angles = kwargs.get('img'), kwargs.get('input_size'), kwargs.get('score'), kwargs.get('rotation_angles')
        img_landmark = img.clone() if kwargs.get('use_landmark_detection') else None
        
        det_img, det_scale, final_input_size = self._prepare_detection_image(img, input_size, 'retinaface')
        ...
        for angle in rotation_angles:
            ...
            for i in ['448', '471', '494', '451', '474', '497', '454', '477', '500']: io_binding.bind_output(i, self.models_processor.device)
            if self.models_processor.device == "cuda": torch.cuda.synchronize()
            ...
            for idx, stride in enumerate([8, 16, 32]):
                scores, bbox_preds, kps_preds = net_outs[idx], net_outs[idx+fmc] * stride, net_outs[idx+fmc*2] * stride
                height, width = input_height // stride, input_width // stride
                key = (height, width, stride)
                if key in self.center_cache: anchor_centers = self.center_cache[key]
                else:
                    anchor_centers = np.stack(np.mgrid[:height, :width][::-1], axis=-1).astype(np.float32)
                    anchor_centers = (anchor_centers * stride).reshape((-1, 2))
                    anchor_centers = np.stack([anchor_centers]*2, axis=1).reshape((-1,2))
                    if len(self.center_cache)<100: self.center_cache[key] = anchor_centers
                pos_inds = np.where(scores>=score)[0]
                bboxes = np.stack([anchor_centers[:, 0] - bbox_preds[:, 0], anchor_centers[:, 1] - bbox_preds[:, 1], anchor_centers[:, 0] + bbox_preds[:, 2], anchor_centers[:, 1] + bbox_preds[:, 3]], axis=-1)
                pos_scores, pos_bboxes = scores[pos_inds], bboxes[pos_inds]
                if angle != 0 and len(pos_bboxes) > 0:
                    points1, points2 = faceutil.trans_points2d(pos_bboxes[:, :2], IM), faceutil.trans_points2d(pos_bboxes[:, 2:], IM)
                    _x1, _y1, _x2, _y2 = points1[:, 0], points1[:, 1], points2[:, 0], points2[:, 1]
                    if angle in (-270, 90): points1, points2 = np.stack((_x1, _y2), axis=1), np.stack((_x2, _y1), axis=1)
                    elif angle in (-180, 180): points1, points2 = np.stack((_x2, _y2), axis=1), np.stack((_x1, _y1), axis=1)
                    elif angle in (-90, 270): points1, points2 = np.stack((_x2, _y1), axis=1), np.stack((_x1, _y2), axis=1)
                    pos_bboxes = np.hstack((points1, points2))
                preds = [item for i in range(0, kps_preds.shape[1], 2) for item in (anchor_centers[:, i%2] + kps_preds[:, i], anchor_centers[:, i%2+1] + kps_preds[:, i+1])]
                ...
                if do_rotation:
                    for i in range(len(pos_kpss)):
                        ...
                        if abs(angle_deg_to_front) > 50.00: pos_scores[i] = 0.0
                        if angle != 0: pos_kpss[i] = faceutil.trans_points2d(pos_kpss[i], IM)
                    pos_inds = np.where(pos_scores>=score)[0]
                    pos_scores, pos_bboxes, pos_kpss = pos_scores[pos_inds], pos_bboxes[pos_inds], pos_kpss[pos_inds]
                kpss_list.append(pos_kpss); bboxes_list.append(pos_bboxes); scores_list.append(pos_scores)
        
        det, kpss, score_values = self._filter_detections_gpu(scores_list, bboxes_list, kpss_list, img.shape[1], img.shape[2], det_scale, kwargs.get('max_num'))
    ...
    def detect_scrdf(self, **kwargs):
        ...
        det_img, det_scale, final_input_size = self._prepare_detection_image(img, input_size, 'scrfd')
        
        scores_list, bboxes_list, kpss_list = [], [], []
        cx, cy = final_input_size[0] / 2, final_input_size[1] / 2
        do_rotation = len(rotation_angles) > 1
        input_name = self.models_processor.models['SCRFD2.5g'].get_inputs()[0].name
        output_names = [o.name for o in self.models_processor.models['SCRFD2.5g'].get_outputs()]
        ...
        for angle in rotation_angles:
            ...
            for name in output_names: io_binding.bind_output(name, self.models_processor.device)
            if self.models_processor.device == "cuda": torch.cuda.synchronize()
    ...
    def detect_yoloface(self, **kwargs):
        ...
        img, score, rotation_angles = kwargs.get('img'), kwargs.get('score'), kwargs.get('rotation_angles')
        img_landmark = img.clone() if kwargs.get('use_landmark_detection') else None
        
        input_size = (640, 640)
        det_img, det_scale, final_input_size = self._prepare_detection_image(img, input_size, 'yolo')
        ...
        for angle in rotation_angles:
            ...
            if keep_indices.any():
                ...
                bboxes_raw = np.stack((bbox_raw[:, 0] - bbox_raw[:, 2] / 2, bbox_raw[:, 1] - bbox_raw[:, 3] / 2,
                                       bbox_raw[:, 0] + bbox_raw[:, 2] / 2, bbox_raw[:, 1] + bbox_raw[:, 3] / 2), axis=-1)
                if angle != 0 and len(bboxes_raw) > 0:
                    points1, points2 = faceutil.trans_points2d(bboxes_raw[:, :2], IM), faceutil.trans_points2d(bboxes_raw[:, 2:], IM)
                    _x1, _y1, _x2, _y2 = points1[:, 0], points1[:, 1], points2[:, 0], points2[:, 1]
                    if angle in (-270, 90): points1, points2 = np.stack((_x1, _y2), axis=1), np.stack((_x2, _y1), axis=1)
                    elif angle in (-180, 180): points1, points2 = np.stack((_x2, _y2), axis=1), np.stack((_x1, _y1), axis=1)
                    elif angle in (-90, 270): points1, points2 = np.stack((_x2, _y1), axis=1), np.stack((_x1, _y2), axis=1)
                    bboxes_raw = np.hstack((points1, points2))
                kpss_raw = np.stack([np.array([[kps[i], kps[i+1]] for i in range(0, len(kps), 3)]) for kps in kps_raw])
                if do_rotation:
                    for i in range(len(kpss_raw)):
                        ...
                        if abs(angle_deg_to_front) > 50.00: score_raw[i] = 0.0
                        if angle != 0: kpss_raw[i] = faceutil.trans_points2d(kpss_raw[i], IM)
                    keep_indices = np.where(score_raw.flatten()>=score)[0]
                    score_raw, bboxes_raw, kpss_raw = score_raw[keep_indices], bboxes_raw[keep_indices], kpss_raw[keep_indices]
                kpss_list.append(kpss_raw); bboxes_list.append(bboxes_raw); scores_list.append(score_raw)
    ...
    def detect_yunet(self, **kwargs):
        ...
        input_size = (640, 640)
        det_img, det_scale, final_input_size = self._prepare_detection_image(img, input_size, 'yunet')
        
        scores_list, bboxes_list, kpss_list = [], [], []
        cx, cy = final_input_size[0] / 2, final_input_size[1] / 2
        do_rotation = len(rotation_angles) > 1
        input_name = self.models_processor.models['YunetN'].get_inputs()[0].name
        output_names = [o.name for o in self.models_processor.models['YunetN'].get_outputs()]
        ...
        for angle in rotation_angles:
            ...
            for idx, stride in enumerate(strides):
                cls_pred, obj_pred, reg_pred, kps_pred = (net_outs[idx].reshape(-1, 1), net_outs[idx + len(strides)].reshape(-1, 1),
                                                          net_outs[idx + len(strides) * 2].reshape(-1, 4), net_outs[idx + len(strides) * 3].reshape(-1, 5 * 2))
                key = (tuple(final_input_size), stride)
                if key in self.center_cache: anchor_centers = self.center_cache[key]
                else:
                    anchor_centers = np.stack(np.mgrid[:(final_input_size[1] // stride), :(final_input_size[0] // stride)][::-1], axis=-1)
                ...
                bboxes = np.stack([(bbox_cxy[:, 0] - bbox_wh[:, 0] / 2.), (bbox_cxy[:, 1] - bbox_wh[:, 1] / 2.),
                                   (bbox_cxy[:, 0] + bbox_wh[:, 0] / 2.), (bbox_cxy[:, 1] + bbox_wh[:, 1] / 2.)], axis=-1)
                ...
                if angle != 0 and len(pos_bboxes) > 0:
                    ...
                    pos_bboxes = np.hstack((points1, points2))
                kpss = np.concatenate([((kps_pred[:, [2 * i, 2 * i + 1]] * stride) + anchor_centers) for i in range(5)], axis=-1)
