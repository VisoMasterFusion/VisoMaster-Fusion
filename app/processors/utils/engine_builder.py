...
class EngineBuilder:
    ...
    def __init__(self, verbose: bool = False, custom_plugin_path: Optional[str] = None, builder_optimization_level: int = 3):
        """
    ...
    def create_network(self, onnx_path: str, opt_shapes: Optional[dict[str, tuple]] = None):
        """
        ...
        for net_input in inputs:
            log.info(f"Input '{net_input.name}' | Shape: {net_input.shape} | Dtype: {net_input.dtype}")
            ...
            if self.batch_size is None:
                ...
                log.warning(f"Inconsistent batch size detected ({self.batch_size} vs {net_input.shape[0]}).")

            if opt_shapes and net_input.name in opt_shapes:
                min_shape, opt_shape, max_shape = opt_shapes[net_input.name]
                log.info(f"Adding optimization profile for '{net_input.name}': Min={min_shape}, Opt={opt_shape}, Max={max_shape}")
                ...
                log.error(f"Input '{net_input.name}' has a dynamic batch size (-1). Optimization shapes must be provided via `opt_shapes`.")
                sys.exit(1)

        for net_output in outputs:
            log.info(f"Output '{net_output.name}' | Shape: {net_output.shape} | Dtype: {net_output.dtype}")
    ...
    def create_engine(self, engine_path: str, precision: str = "fp16") -> bool:
        ...
        if self.network is None:
            log.error("TensorRT network has not been created. Call `create_network` first.")
            return False
        ...
        if precision == "fp16":
            if self.builder.platform_has_fast_fp16:
                ...
                log.warning("FP16 is not supported natively on this platform/device. Using FP32.")
        elif precision == "int8":
            # INT8 requires an IInt8Calibrator, which complicates the example.
            if self.builder.platform_has_fast_int8:
                self.config.set_flag(trt.BuilderFlag.INT8)
            else:
                log.warning("INT8 is not supported natively on this platform/device. Using FP32.")
        elif precision == "fp32":
            pass # Default
...
def change_extension(file_path: Union[str, Path], new_extension: str, version: Optional[str] = None) -> str:
    """
...
def onnx_to_trt(onnx_model_path: str, trt_model_path: Optional[str] = None, 
                precision: str = "fp16", custom_plugin_path: Optional[str] = None, 
                verbose: bool = False, opt_shapes: Optional[dict[str, tuple]] = None) -> None:
    """
