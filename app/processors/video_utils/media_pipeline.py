...
class MediaPipeline(QObject):
    ...
    def store_frame_to_display(self, frame_number: int, frame: numpy.ndarray) -> None:
        ...
        is_future_wrap_frame = (self._wrap_frame_target != -1 and frame_number < self._wrap_frame_target)

        if self.vp.file_type == "video" and frame_number < self.vp.next_frame_to_display and not draining_tail and not is_future_wrap_frame:
            del frame
            return
        ...
        while len(self.frames_to_display) > self.max_frames_to_display_size:
            ...
            for k in list(self.frames_to_display.keys()):
                if self._wrap_frame_target != -1:
                    ...
                    if k < self.vp.next_frame_to_display and (self.vp.next_frame_to_display - k) < (self.vp.max_frame_number / 2):
                        stale_keys.append(k)
                else:
    ...
    def start_feeder(self, mode: str, recording: bool) -> None:
        ...
        print(f"[INFO] Starting feeder thread (Mode: {mode}, Recording: {recording})...")
    ...
    def _feeder_loop(self) -> None:
        ...
        print(f"[INFO] Feeder thread started (Mode: {self.vp.file_type}, Segments: {self.vp.is_processing_segments}).")
        try:
            if self.vp.file_type == "webcam":
                self._feed_webcam()
            else:
    ...
    def _feed_video_loop(self) -> None:
        ...
        def stop_flag_check():
            return self.vp.is_processing_segments if is_segment_mode else self.vp.processing

        print(f"[INFO] Feeder: Starting video loop (Mode: {'Segment' if is_segment_mode else 'Standard'}).")
            ...
            try:
                ...
                if is_segment_mode:
                    ...
                    if self.vp.current_frame_number > self.vp.current_segment_end_frame:
                        print(f"[INFO] Feeder: Reached end of segment {self.vp.current_segment_index + 1}. Stopping feed.")
                        break
                    ...
                    if self.vp.current_frame_number > self.vp.current_segment_end_frame:
                        ...
                        is_playback_loop_enabled = self.main_window.control.get("VideoPlaybackLoopToggle", False)
                        ...
                        if next_idx < len(self.vp.segments_to_process):
                            ...
                            print("[INFO] Feeder: Reached end of final playback segment. Stopping feed.")
                            break
                        ...
                        with self.state_lock:
                            if not hasattr(self, 'segment_jumps'):
                                self.segment_jumps = {}
                        ...
                        print(f"[INFO] Feeder: Segment jump registered ({jump_from} -> {jump_to}).")
                    ...
                    if self.vp.current_frame_number > self.vp.max_frame_number:
                        ...
                        if is_playback_loop_enabled and not self.vp.recording:
                            print("[INFO] Feeder: End of media, flowing seamlessly back to start.")
                            with self.state_lock:
                                self._wrap_frame_target = self.vp.current_frame_number - 1
                ...
                # 2. Buffer control (VRAM & RAM Safety Net)
                if len(self.frames_to_display) >= self.max_frames_to_display_size:
                    time.sleep(0.005)
                    continue

                in_flight_frames = len(self.frames_to_display) + self.vp.frame_queue.qsize()

                # OPTIMIZATION RAM: Absolute Available Memory Safety Net to prevent OS freezing
                MIN_FREE_RAM_BYTES = 2.5 * 1024 * 1024 * 1024  # 2.5 GB
                min_safe_buffer = min(self.vp.num_threads * 2, 8)

                if in_flight_frames > min_safe_buffer and psutil.virtual_memory().available < MIN_FREE_RAM_BYTES:
                    time.sleep(0.05) 
                    continue
                ...
                if (is_segment_mode or self.vp.recording) and not self.vp.ffmpeg_input_sp and self.vp.current_frame_number in self.main_window.dropped_frames:
                    self._mark_skipped_frame(self.vp.current_frame_number, "manual_drop")
                    self.vp.current_frame_number += 1
                    misc_helpers.seek_frame(self.vp.media_capture, self.vp.current_frame_number)
                    continue
                ...
                if not ret:
                    if self.vp.ffmpeg_input_sp:
                        remaining_frames = self.vp.max_frame_number - self.vp.current_frame_number
                        eof_like = (self.vp.current_frame_number >= self.vp.max_frame_number - TAIL_TOLERANCE or remaining_frames <= MAX_CONSECUTIVE_ERRORS)
                        if eof_like:
                            ...
                            print(f"[WARN] Feeder: FFmpeg input stream terminated early at {self.vp.current_frame_number}. Treating as corrupted input.")
                        with self.state_lock:
                            self.vp.next_frame_to_display = self.vp.max_frame_number + 1
                        break

                    fn = self.vp.current_frame_number
                    if (is_segment_mode or is_playing_segments) and self.vp.current_segment_end_frame is not None:
                        if fn >= self.vp.current_segment_end_frame - TAIL_TOLERANCE:
                            if is_playing_segments:
                                print(f"[INFO] Feeder: Read failure near playback segment tail (frame={fn}). Forcing segment jump.")
                                with self.state_lock:
                                    self.vp.current_frame_number = self.vp.current_segment_end_frame + 1
                                continue
                            else:
                                with self.state_lock:
                                    self.vp.next_frame_to_display = self.vp.current_segment_end_frame + 1
                                    self.vp.current_frame_number = self.vp.current_segment_end_frame + 1
                                print(f"[INFO] Feeder: Treat read failure near segment tail as EOF (frame={fn}).")
                                break

                    if not is_segment_mode and fn >= self.vp.max_frame_number - TAIL_TOLERANCE:
                        is_playback_loop_enabled = self.main_window.control.get("VideoPlaybackLoopToggle", False)
                        if is_playback_loop_enabled and not self.vp.recording:
                            print(f"[INFO] Feeder: Read failure near file end (frame={fn}/{self.vp.max_frame_number}), flowing seamlessly to start.")
                            with self.state_lock:
                                self._wrap_frame_target = fn - 1
                            self.vp.current_frame_number = 0
                            misc_helpers.seek_frame(self.vp.media_capture, 0)
                            self.consecutive_read_errors = 0 # Clear skip tracking to prevent abort
                            continue
                        else:
                            print(f"[INFO] Feeder: Read failure near file end (frame={fn}/{self.vp.max_frame_number}), treating as EOF.")
                            with self.state_lock:
                                self.vp.next_frame_to_display = self.vp.max_frame_number + 1
                            break
                    ...
                    if self.consecutive_read_errors > MAX_CONSECUTIVE_ERRORS:
                        print(f"[INFO] Feeder: Too many consecutive read errors ({self.consecutive_read_errors}), likely reached EOF. Stopping.")
                        try:
                    ...
                    print(f"[WARN] Feeder: Skipping unreadable frame {self.vp.current_frame_number} (Total skipped: {self.total_skipped_frames}).")
                    self.vp.current_frame_number += 1
                    misc_helpers.seek_frame(self.vp.media_capture, self.vp.current_frame_number)
                    continue  
                ...
                with self.state_lock:
                    if marker_data and marker_data != last_marker_data:
                        print(f"[INFO] Frame {frame_num_to_process} is a marker. Updating feeder state.")
                        self.feeder_parameters = copy.deepcopy(marker_data["parameters"])
                        self.feeder_control = {}
                        ...
                        if "control" in marker_data and isinstance(marker_data["control"], dict):
                            self.feeder_control.update(cast(ControlTypes, marker_data["control"]).copy())
                        last_marker_data = marker_data
                        self.ui_state_is_dirty = True

                    if getattr(self, "ui_state_is_dirty", True) or not hasattr(self, "_cached_params"):
    ...
    def _feed_webcam(self) -> None:
        ...
        while self.vp.processing:
            try:
                in_flight_frames = len(self.webcam_frames_to_display.queue) + self.vp.frame_queue.qsize()
                if in_flight_frames >= self.vp.max_display_buffer_size:
                    time.sleep(0.005) 
                    continue

                ret, frame_bgr = misc_helpers.read_frame(self.vp.media_capture, 0, preview_target_height=None)
                if not ret:
                ...
                with self.main_window.models_processor.model_lock:
                    if getattr(self, "ui_state_is_dirty", True) or not hasattr(self, "_webcam_cached_params"):
                        self._webcam_cached_params = fast_state_copy(self.main_window.parameters)
                        self._webcam_cached_control = fast_state_copy(self.main_window.control)
                        self.ui_state_is_dirty = False
                        print("[INFO] Global State changed : Dirty flag cleared")

                    local_params_for_worker = fast_state_copy(self._webcam_cached_params)
                    local_control_for_worker = fast_state_copy(self._webcam_cached_control)
    ...
    def _check_preroll_and_start_playback(self) -> None:
        ...
        is_feeder_done = not self.feeder_thread.is_alive() if self.feeder_thread else False
        ...
        if len(self.frames_to_display) >= self.vp.preroll_target or is_feeder_done:
            ...
            print(f"[INFO] Preroll buffer ready ({len(self.frames_to_display)} frames). Starting playback components...")
            self._start_synchronized_playback()
        else:
            print(f"[INFO] Buffering... {len(self.frames_to_display)} / {self.vp.preroll_target}")
    ...
    def _handle_tail_drain_wait(self, frame_number_to_display: int) -> bool:
        ...
        if now_sec - self.vp.tail_pending_stall_start_sec >= TAIL_PENDING_STALL_TIMEOUT_SEC:
            self.vp.tail_force_finalize_due_to_stall = True
            self.vp.tail_pending_stall_start_sec = 0.0
            print(f"[WARN] Tail-drain stalled for too long ({TAIL_PENDING_STALL_TIMEOUT_SEC:.1f}s). Forcing finalization.")
            return True
    ...
    def display_next_frame(self) -> None:
        ...
        if self.vp.file_type == "video":
            if self.vp.is_processing_segments:
                if self.vp.current_segment_end_frame is not None and self.vp.next_frame_to_display > self.vp.current_segment_end_frame:
                    print(f"[INFO] Segment {self.vp.current_segment_index + 1} end frame ({self.vp.current_segment_end_frame}) reached.")
                    self.vp.stop_current_segment()  
                    return
            else:
                hit_eof = False
                with self.state_lock:
                    if self._wrap_frame_target != -1 and self.vp.next_frame_to_display > self._wrap_frame_target:
                        hit_eof = True
                ...
                if hit_eof:
                    if self.vp.recording:
                        pending_tasks = self.vp._safe_unfinished_tasks()
                        if not self.frames_to_display and (pending_tasks == 0 or self.vp.tail_force_finalize_due_to_stall):
                            print("[INFO] End of media reached.")
                            should_finalize_default_recording = True
                    elif is_playback_loop_enabled:
                        print("[INFO] Metronome: End of media reached. Seamless loop wrapping active.")
                        with self.state_lock:
                            if is_playing_segments and self.vp.segments_to_process:
                                self.vp.next_frame_to_display = self.vp.segments_to_process[0][0]
                                self.vp.current_segment_index = 0
                                self.vp.current_segment_end_frame = self.vp.segments_to_process[0][1]
                            else:
                                self.vp.next_frame_to_display = 0
                            self._wrap_frame_target = -1
                        
                        self.main_window.videoSeekSlider.blockSignals(True)
                        self.main_window.videoSeekSlider.setValue(self.vp.next_frame_to_display)
        ...
        if self.vp.file_type == "webcam":
            ...
            while frame_number_to_display in self.skipped_frames and frame_number_to_display <= self.vp.max_frame_number:
                frame_number_to_display += 1

            if frame_number_to_display > original_frame:
                skipped_count = frame_number_to_display - original_frame
                print(f"[INFO] Display: Advancing past {skipped_count} skipped frame(s), jumping to frame {frame_number_to_display}")
        ...
        if self.vp.is_processing_segments or self.vp.recording:
            if self.vp.encoder.is_running():
                if self.vp.encoder.write_frame(frame):
                    ...
                    log_prefix = f"segment {self.vp.current_segment_index + 1}" if self.vp.is_processing_segments else "recording"
                    print(f"[WARN] Error writing frame {frame_number_to_display} to FFmpeg encoder during {log_prefix}.")
            else:
                log_prefix = f"segment {self.vp.current_segment_index + 1}" if self.vp.is_processing_segments else "recording"
                print(f"[WARN] FFmpeg encoder not available for {log_prefix} when trying to write frame {frame_number_to_display}.")

        if self.vp.file_type != "webcam":
            if frame_number_to_display in self.main_window.markers:
                with self.main_window.models_processor.model_lock:
                    video_control_actions.update_parameters_and_control_from_marker(self.main_window, frame_number_to_display)
                    video_control_actions.update_widget_values_from_markers(self.main_window, frame_number_to_display)
        ...
        if self.vp._used_ffmpeg_cap and self.vp.fps > 0 and self.vp.recording_source_fps > 0:
            src_slider_max = self.main_window.videoSeekSlider.maximum()
            slider_display_frame = min(self.vp.output_to_source_frame(frame_number_to_display), src_slider_max)
            
        graphics_view_actions.update_graphics_view(self.main_window, pixmap, slider_display_frame)

        self.main_window.models_processor.check_deferred_unloads(frame_number_to_display)
        if self.vp.file_type != "webcam":
            jump_occurred = False
            if hasattr(self, 'segment_jumps') and frame_number_to_display in self.segment_jumps:
    ...
    def start_live_sound(self) -> None:
        ...
        playback_rate = 1.0
        if self.main_window.control.get("VideoPlaybackCustomFpsToggle", False) and not self.vp.recording:
        ...
        volume = float(self.main_window.control.get('LiveSoundVolumeDecimalSlider', 1.0))
        ...
        print(f"[INFO] Native audio started at {seek_time_ms}ms with {playback_rate:.2f}x speed.")
    ...
    def _start_synchronized_playback(self) -> None:
        ...
        if self.main_window.liveSoundButton.isChecked() and not self.vp.recording:
            ...
            print(f"[INFO] Waiting {AUDIO_STARTUP_LATENCY_MS}ms for audio buffer to initialize...")
