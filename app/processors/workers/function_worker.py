...
class FunctionWorker:
    ...
    def switch_providers_priority(self, provider_name: str) -> str:
        ...
        if hasattr(self.face_editors, "lp_mask_crop"):
            self.face_editors.lp_mask_crop = self.face_editors.lp_mask_crop.to(self.mp.device)
            
        if hasattr(self.face_denoiser, "alphas_cumprod_torch"):
            self.face_denoiser.alphas_cumprod_torch = self.face_denoiser.alphas_cumprod_torch.to(self.mp.device)
    ...
    def run_inswapper(self, image: np.ndarray, embedding: np.ndarray, output: Any) -> None:
        self.face_swappers.run_inswapper(image, embedding, output)

    def run_inswapper_batched(self, images: List[np.ndarray], embedding: np.ndarray, output: Any) -> None:
        self.face_swappers.run_inswapper_batched(images, embedding, output)

    def calc_swapper_latent_iss(self, source_embedding: np.ndarray, version: str = "A") -> np.ndarray:
        return self.face_swappers.calc_swapper_latent_iss(source_embedding, version)

    def run_iss_swapper(self, image: np.ndarray, embedding: np.ndarray, output: Any, version: str = "A") -> None:
        self.face_swappers.run_iss_swapper(image, embedding, output, version)

    def calc_swapper_latent_simswap512(self, source_embedding: np.ndarray) -> np.ndarray:
        return self.face_swappers.calc_swapper_latent_simswap512(source_embedding)

    def run_swapper_simswap512(self, image: np.ndarray, embedding: np.ndarray, output: Any) -> None:
    ...
    def run_swapper_ghostface(
        self, image: np.ndarray, embedding: np.ndarray, output: Any, swapper_model: str = "GhostFace-v2"
    ) -> None:
    ...
    def run_swapper_cscs(self, image: np.ndarray, embedding: np.ndarray, output: Any) -> None:
    ...
    def lp_motion_extractor(self, img: np.ndarray, face_editor_type: str = "Human-Face", **kwargs: Any) -> dict:
        return self.face_editors.lp_motion_extractor(img, face_editor_type, **kwargs)

    def lp_appearance_feature_extractor(self, img: np.ndarray, face_editor_type: str = "Human-Face") -> Any:
    ...
    def perform_recast_compose_driven_keypoints(
        ...
        return self.perform_recast.compose_driven_keypoints(source_info, exp_d, **kwargs)
    ...
    def run_CLIPs(self, img: np.ndarray, CLIPText: str, CLIPAmount: float) -> np.ndarray:
        return self.face_masks.run_CLIPs(img, CLIPText, CLIPAmount)

    def apply_occlusion(self, img: np.ndarray, amount: float) -> np.ndarray:
        return self.face_masks.apply_occlusion(img, amount)

    def apply_dfl_xseg(
        self, img: np.ndarray, amount: float, mouth: Any, parameters: dict, inner_mouth_mask: Any
    ) -> np.ndarray:
    ...
    def process_masks_and_masks(
        self, swap_restorecalc: np.ndarray, original_face_512: np.ndarray, parameters: dict, control: dict
    ) -> Any:
    ...
    def get_kv_map_for_face(self, image: Any, unload_after: bool = True) -> Dict[str, Dict[str, torch.Tensor]]:
    ...
    def apply_denoiser_unet(
        ...
        return self.face_denoiser.apply_denoiser_unet(
            image_cxhxw_uint8, reference_kv_map, use_reference_exclusive_path,
            denoiser_mode, denoiser_single_step_t, denoiser_ddim_steps,
            denoiser_cfg_scale, denoiser_ddim_eta, base_seed,
            latent_sharpening_strength, color_transfer, color_transfer_mode,
            color_mask, blend_mask
        )

    def run_vae_encoder(self, image_srgb_float_minus1_1_batched: torch.Tensor, encoded_latent_direct_vae_out_bchw: torch.Tensor) -> None:
        self.face_restorers.run_vae_encoder(image_srgb_float_minus1_1_batched, encoded_latent_direct_vae_out_bchw)

    def run_ref_ldm_unet(self, x_noisy_plus_lq_latent: torch.Tensor, timesteps_tensor: torch.Tensor, is_ref_flag_tensor: torch.Tensor, use_reference_exclusive_path_globally_tensor: torch.Tensor, kv_tensor_map: Dict | None, output_unet_tensor: torch.Tensor) -> None:
        self.face_restorers.run_ref_ldm_unet(x_noisy_plus_lq_latent, timesteps_tensor, is_ref_flag_tensor, use_reference_exclusive_path_globally_tensor, kv_tensor_map, output_unet_tensor)

    def run_vae_decoder(self, latent_for_vae_decoder: torch.Tensor, decoded_image_normalized_bchw: torch.Tensor) -> None:
