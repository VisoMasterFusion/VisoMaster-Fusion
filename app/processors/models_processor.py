...
class ModelsProcessor(QtCore.QObject):
    ...
    def _check_tensorrt_cache(self, model_name: str, onnx_path: str) -> bool:
        ...
        try:
            ...
            # Support both strictly applied UI names and standard ONNX file names
            possible_prefixes = list(set([model_name, base_onnx_name]))
            ...
            for prefix in possible_prefixes:
                ...
                if os.path.exists(ctx_file_path):
                    ...
                    # Look for the engine name embedded in the context file using a broader regex
                    # capturing any alphanumeric, dash, or underscore sequence ending in .engine
                    ...
                    # Depending on the ORT version and pathing, the engine can be in the root cache
                    # directory or nested in a subdirectory matching the folder name.
            ...
            return False  # No valid engine found after exhausting all prefixes
    ...
    def _clean_tensorrt_cache(
        ...
        # Extract the explicitly set prefix, falling back to base ONNX name
        target_prefix = trt_options.get("trt_engine_cache_prefix", base_onnx_name)

        # 1. Try to read the context file to find the specific engine file before deleting it
        engine_file_paths_to_check = []
        possible_prefixes = list(set([target_prefix, base_onnx_name]))
        ...
        for prefix in possible_prefixes:
            ...
            # 2. Delete the context file safely
            if os.path.exists(ctx_file_path) and os.path.isfile(ctx_file_path):
                try:
    ...
    def load_model(self, model_name, session_options=None):
        ...
        with self.model_lock:
            ...
            # --- NEW: EXPLICIT ENGINE CACHE PREFIX ---
            # Set a unique cache prefix for this model to prevent collisions
            # and guarantee highly predictable tensor cleanup
