...
class FaceLandmarkDetectors:
    ...
    def __init__(self, models_processor: 'ModelsProcessor'):
        ...
        self.detector_map = {
            '5': {'model_name': 'FaceLandmark5', 'function': self.detect_face_landmark_5},
            '68': {'model_name': 'FaceLandmark68', 'function': self.detect_face_landmark_68},
            '3d68': {'model_name': 'FaceLandmark3d68', 'function': self.detect_face_landmark_3d68},
            '98': {'model_name': 'FaceLandmark98', 'function': self.detect_face_landmark_98},
            '106': {'model_name': 'FaceLandmark106', 'function': self.detect_face_landmark_106},
            '203': {'model_name': 'FaceLandmark203', 'function': self.detect_face_landmark_203},
            '478': {'model_name': 'FaceLandmark478', 'function': self.detect_face_landmark_478}
        }
    ...
    def run_detect_landmark(self, img, bbox, det_kpss, detect_mode='203', score=0.5, from_points=False):
        ...
        kpss_5, kpss, scores = detection_function(img, bbox=bbox, det_kpss=det_kpss, from_points=from_points)

        # Filter the final results based on the provided confidence score.
        if len(kpss_5) > 0 and len(scores) > 0:
            if np.mean(scores) >= score:
                return kpss_5, kpss, scores
        elif len(kpss_5) > 0:  # If no scores are returned by the model, pass through the landmarks.
            return kpss_5, kpss, scores
    ...
    def _ensure_landmark_5_anchors(self):
        ...
        feature_maps, min_sizes, steps, image_size = [[64, 64], [32, 32], [16, 16]], [[16, 32], [64, 128], [256, 512]], [8, 16, 32], 512
        anchors = []
        for k, f in enumerate(feature_maps):
            for i, j in product(range(f[0]), range(f[1])):
                for min_size in min_sizes[k]:
                    s_kx, s_ky = min_size / image_size, min_size / image_size
                    dense_cx, dense_cy = [x * steps[k] / image_size for x in [j + 0.5]], [y * steps[k] / image_size for y in [i + 0.5]]
    ...
    def _prepare_crop(self, img, bbox, det_kpss, from_points, target_size, warp_mode=None, scale=1.5, vy_ratio=0.0):
        """
        ...
        if not from_points:
            ...
            if det_kpss is None or len(det_kpss) == 0: return None, None, None
            ...
            if warp_mode in ['arcface128', 'arcfacemap']:
                 aimg, M = faceutil.warp_face_by_face_landmark_5(img, det_kpss, image_size=target_size, mode=warp_mode, interpolation=v2.InterpolationMode.BILINEAR)
                 IM = faceutil.invertAffineTransform(M)
            else: # Default for models like landmark_203 which use a more generic warp.
                aimg, M, IM = faceutil.warp_face_by_face_landmark_x(img, det_kpss, dsize=target_size, scale=scale, vy_ratio=vy_ratio, interpolation=v2.InterpolationMode.BILINEAR)
        return aimg, M, IM

    def _run_onnx_binding(self, model_name: str, input_bindings: Dict[str, torch.Tensor], output_names: List[str]) -> List[np.ndarray]:
        """
        ...
        for name, tensor in input_bindings.items():
            io_binding.bind_input(name=name, device_type=self.models_processor.device, device_id=0, element_type=np.float32, shape=tensor.size(), buffer_ptr=tensor.data_ptr())
    ...
    def detect_face_landmark_5(self, img, bbox, det_kpss, from_points=False):
        ...
        if (width, height) not in self.landmark_5_scale1_cache:
            self.landmark_5_scale1_cache[(width, height)] = torch.tensor([width, height]*5, dtype=torch.float32, device=self.models_processor.device)
        scale1 = self.landmark_5_scale1_cache[(width, height)]

        # Run inference.
        net_outs = self._run_onnx_binding('FaceLandmark5', {'input': image}, ['conf', 'landmarks'])
        conf, landmarks = torch.from_numpy(net_outs[0]).to(self.models_processor.device), torch.from_numpy(net_outs[1]).to(self.models_processor.device)
        ...
        landmarks = torch.cat([ priors[:, :2] + pre[:, i:i+2] * 0.1 * priors[:, 2:] for i in range(0, 10, 2) ], dim=1) * scale1
    ...
    def detect_face_landmark_68(self, img, bbox, det_kpss, from_points=False):
        ...
        crop_image = torch.div(crop_image.to(dtype=torch.float32), 255.0).unsqueeze(0).contiguous()

        net_outs = self._run_onnx_binding('FaceLandmark68', {'input': crop_image}, ['landmarks_xyscore', 'heatmaps'])
        face_landmark_68, face_heatmap = net_outs[0], net_outs[1]
        
        # Post-process: scale, transform, and reshape landmarks.
        face_landmark_68 = (face_landmark_68[:, :, :2][0] / 64.0).reshape(1, -1, 2) * 256.0
        face_landmark_68 = cv2.transform(face_landmark_68, cv2.invertAffineTransform(affine_matrix)).reshape(-1, 2)
    ...
    def detect_face_landmark_3d68(self, img, bbox, det_kpss, from_points=False):
        aimg, _, IM = self._prepare_crop(img, bbox, det_kpss, from_points, target_size=192, warp_mode='arcface128')
        if aimg is None: return [], [], []

        aimg = self.models_processor.normalize(aimg.to(dtype=torch.float32)).unsqueeze(0).contiguous()
        ...
        if 68 < pred.shape[0]: pred = pred[-68:]
        pred[:, 0:2] = (pred[:, 0:2] + 1) * 96.0 # Scale to image size (192/2)
        if pred.shape[1] == 3: pred[:, 2] *= 96.0
    ...
    def detect_face_landmark_98(self, img, bbox, det_kpss, from_points=False):
        ...
        if not from_points:
            ...
            if crop_image is not None: h, w = crop_image.size(1), crop_image.size(2)

        if crop_image is None: return [], [], []
        
        crop_image = torch.div(crop_image.to(dtype=torch.float32), 255.0).unsqueeze(0).contiguous()
        landmarks_xyscore = self._run_onnx_binding('FaceLandmark98', {'input': crop_image}, ['landmarks_xyscore'])[0]

        if len(landmarks_xyscore) > 0:
            one_face_landmarks = landmarks_xyscore[0]
            landmark_score, landmark = one_face_landmarks[:, 2], one_face_landmarks[:, :2]
            ...
            if not from_points:
                ...
                landmark[:, 0] *= w
                landmark[:, 1] *= h
                landmark = faceutil.trans_points2d(landmark, faceutil.invertAffineTransform(M))
    ...
    def detect_face_landmark_106(self, img, bbox, det_kpss, from_points=False):
        ...
        pred = pred.reshape((-1, 3)) if pred.shape[0] >= 3000 else pred.reshape((-1, 2))
        if 106 < pred.shape[0]: pred = pred[-106:]
        
        pred[:, :2] = (pred[:, :2] + 1) * 96.0
        if pred.shape[1] == 3: pred[:, 2] *= 96.0

        pred = faceutil.trans_points(pred, IM)
        pred_5 = faceutil.convert_face_landmark_106_to_5(pred) if pred is not None else []
        return pred_5, pred, []
    ...
    def detect_face_landmark_203(self, img, bbox, det_kpss, from_points=False):
        ...
        warp_mode = None if (from_points and det_kpss is not None and det_kpss.shape[0] > 5) else 'arcface128'
        aimg, M, IM = self._prepare_crop(img, bbox, det_kpss, from_points, target_size=224, warp_mode=warp_mode, scale=1.5, vy_ratio=-0.1)
        if aimg is None: return [], [], []
        if IM is None: IM = faceutil.invertAffineTransform(M)

        aimg = torch.div(aimg.to(dtype=torch.float32), 255.0).unsqueeze(0).contiguous()
        
        out_lst = self._run_onnx_binding('FaceLandmark203', {'input': aimg}, ['output', '853', '856'])
        out_pts = out_lst[2].reshape((-1, 2)) * 224.0 # The third output contains the landmarks.
        
        out_pts = faceutil.trans_points(out_pts, IM)
        out_pts_5 = faceutil.convert_face_landmark_203_to_5(out_pts) if out_pts is not None else []
        return out_pts_5, out_pts, []

    def detect_face_landmark_478(self, img, bbox, det_kpss, from_points=False):
        aimg, _, IM = self._prepare_crop(img, bbox, det_kpss, from_points, target_size=256, warp_mode='arcfacemap', scale=1.5)
        if aimg is None: return [], [], []

        aimg = torch.div(aimg.to(dtype=torch.float32), 255.0).unsqueeze(0).contiguous()
        
        net_outs = self._run_onnx_binding('FaceLandmark478', {'input_12': aimg}, ['Identity', 'Identity_1', 'Identity_2'])
        ...
        if len(landmarks) > 0:
            ...
            landmark_for_score = torch.from_numpy(np.expand_dims(landmark_for_score, axis=0).astype(np.float32)).to(self.models_processor.device)
            self._run_onnx_binding('FaceBlendShapes', {'input_points': landmark_for_score}, ['output'])
